...
## Telegram Bot Messaging Profile

当通过 Telegram 渠道接收消息时，Bot 遵循以下限制：

- **只使用 Read、Grep、Glob 工具**，不执行 Bash、Write、Edit 操作。
- 回答风格：简洁、直接、1-3 句即可。
- 所有知识查询优先搜索 `knowledge/articles/` 目录。
- 不允许修改项目文件或执行本地命令。
- 不确定的答案标注"待确认"，不编造。
...
```
knowledge/articles/    → 标准知识条目 JSON
knowledge/raw/         → 原始采集数据
knowledge/metrics/     → 成本统计
knowledge/pending_review/ → 待人工审核
```
